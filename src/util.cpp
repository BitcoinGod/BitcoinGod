...
void OpenDebugLog()
{
    ...
    assert(fileoutVM == nullptr); // godcoin:contract
    fs::path pathDebug = GetDataDir() / "debug.log";
    fs::path pathDebugVM = GetDataDir() / "vm.log"; // godcoin:contract
    fileout = fsbridge::fopen(pathDebug, "a");
    fileoutVM = fsbridge::fopen(pathDebugVM, "a");// godcoin:contract
    if (fileout) {
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinGod";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinGod";
#else
    // Unix
    return pathRet / ".bitcoingod";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitcoinGod Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitcoinGod Core developers";
    }
